...
import { isHex, u8aToHex, bufferToU8a, stringToU8a, hexToU8a, u8aToString } from '@polkadot/util';
import { keyExtractSuri, mnemonicGenerate, mnemonicValidate, randomAsU8a, naclKeypairFromSeed, mnemonicToMiniSecret } from '@polkadot/util-crypto';
...
import { MnemonictoSeedV1 } from './utils';
...
function addressFromSeed (phrase: string, derivePath: string, pairType: KeypairType, seedType?: SeedType): string {
  if (seedType && seedType === 'bip') {
    ...
    if (u8aToHex(currentkeypair.secretKey.subarray(0, 32))) {
      return keyring
        .createFromUri(`${u8aToHex(currentkeypair.secretKey.subarray(0, 32))}${derivePath}`, {}, pairType)
        .address;
    }
  } else {
    return keyring
...
function Create ({ className = '', onClose, onStatusChange, seed: propsSeed, type: propsType }: Props): React.ReactElement<Props> {
  ...
  const _onChangeSeed = (newSeed: string) => {
