...
import { Button, Dropdown, Input, Table, Tabs } from '@polkadot/react-components';
...
function VanityApp ({ basePath, className = '', onStatusChange }: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={className}>
      ...
      {matches.length !== 0 && (
        <>
          <article
            className='warning centered'>{t<string>('Ensure that you utilized the "Save" functionality before using a generated address to receive funds. Without saving the address any funds and the associated seed any funds sent to it will be lost.')}</article>
          <Table
