...
import { Badge, ChainImg, Icon } from '@polkadot/react-components';
...
function Item({ className = '', isToplevel, isBrowser = false, route: { Modal, href, icon, name, text, useCounter = DUMMY_COUNTER } }: Props): React.ReactElement<Props> {
  ...
  return (
    <li
      className={`${className}${count ? ' withCounter' : ''} ${isToplevel ? 'topLevel  highlight--color-contrast' : 'highlight--hover-color'}`}>
      <a
